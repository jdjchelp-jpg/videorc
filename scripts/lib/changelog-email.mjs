...
export function renderChangelogEmail(entry, { webBaseUrl = DEFAULT_WEB_BASE_URL } = {}) {
  ...
  const downloadUrl = `${base}/account/download`
  return {
    subject: `Videorc ${formatVersion(entry.version)} — ${entry.summary}`,
    html: renderHtml(entry, { downloadUrl, releaseUrl }),
    text: renderText(entry, { downloadUrl, releaseUrl })
  }
}
...
function renderHtml(entry, { downloadUrl, releaseUrl }) {
  ...
  return [
    ...
    `<p style="margin:0;font-size:12px;letter-spacing:0.08em;text-transform:uppercase;color:#9a9aa2;">Videorc changelog · ${escapeHtml(entry.date)}</p>`,
    `<h1 style="margin:12px 0 0;font-size:28px;line-height:1.2;font-weight:600;color:#ffffff;">Videorc ${escapeHtml(formatVersion(entry.version))}</h1>`,
    ...
    `<a href="${escapeAttribute(downloadUrl)}" style="display:inline-block;margin:28px 0 0;padding:12px 24px;border-radius:8px;background:#ffffff;color:#0b0b0e;font-size:15px;font-weight:600;text-decoration:none;">Update Videorc</a>`,
...
function renderText(entry, { downloadUrl, releaseUrl }) {
  return [
    `Videorc ${formatVersion(entry.version)} — ${entry.date}`,
    '',
    entry.summary,
    '',
    entry.title,
