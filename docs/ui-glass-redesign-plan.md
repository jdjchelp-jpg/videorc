...
**Non-goals.**
- No light mode. The app becomes dark-native; the theme toggle is removed.
...
3. **Dark-native tokens.** `:root` carries the skill's dark glass values directly; the `.dark` block and `next-themes`/toggle are deleted. Mapping (skill → shadcn vars):
...
- **S0 — Tokens, fonts, dark-native.** Rewrite `styles.css` tokens per Foundation 2–3; remove fontsource imports/deps; delete theme-toggle + next-themes wiring; force dark root. The app will look half-migrated (old layouts, new skin) — acceptable, slices are additive. *Check:* screenshot every tab via the `capture-page` smoke command; no light surfaces remain; fonts render SF Pro.
